...
// homens que não gostaram
float obterPorcentagemB(int qnt_entrevistado, char **sexo, char **opiniao) {
  float porcentagem = 0;
  for (size_t i = 0; i < qnt_entrevistado; i++) {
    porcentagem = (strcmp(sexo[i], "masculino\n") == 0 &&
                   strcmp(opiniao[i], "não gostou\n") == 0)
                      ? porcentagem + 1
                      : porcentagem;
  }
  return (float)(porcentagem * 100 /
...
int main(void) {
  ...
  printf("A porcentagem de homens que não gostaram foi: %.2f%%\n",
         obterPorcentagemB(qnt_entrevistado, sexo, opiniao));
  return 0;
}
